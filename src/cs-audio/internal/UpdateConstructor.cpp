...
void UpdateConstructor::updateAll(std::shared_ptr<std::vector<std::shared_ptr<SourceBase>>> sources,
    std::shared_ptr<std::vector<std::shared_ptr<SourceGroup>>>                              groups,
    std::shared_ptr<AudioController> audioController) {

  // possible improvement: disable mixing with group settings if there are no group updates ->
  // change in createUpdateList() required
  ...
  if (containsRemove(audioController->mUpdateSettings)) {
    ...
    for (auto sourcePtr : *sources) {
      ...
      if (!sourcePtr->mGroup.expired() &&
          containsRemove(sourcePtr->mGroup.lock()->mUpdateSettings)) {
        rebuildPlaybackSettings(audioController, sourcePtr);
        continue;
      }
      ...
      auto finalSettings =
          std::make_shared<std::map<std::string, std::any>>(*(audioController->mUpdateSettings));
      ...
      if (!sourcePtr->mGroup.expired()) {
        ...
        // take group settings
        auto finalGroup = std::make_shared<std::map<std::string, std::any>>(
            *(sourcePtr->mGroup.lock()->mUpdateSettings));
      ...
      auto failedSettings = mProcessingStepsManager->process(
          sourcePtr, audioController->getControllerId(), finalSettings);
  ...
  SettingsMixer::OverrideAdd_A_with_B(
      audioController->mCurrentSettings, audioController->mUpdateSettings);
...
void UpdateConstructor::updateGroups(
  ...
  for (auto sourcePtr : *sources) {
    ...
    // take group settings
    auto finalSettings = std::make_shared<std::map<std::string, std::any>>(
        *(sourcePtr->mGroup.lock()->mUpdateSettings));
...
void UpdateConstructor::updateSources(
  ...
  for (auto sourcePtr : *sources) {
    ...
    // run finalSetting through pipeline
    auto failedSettings = mProcessingStepsManager->process(
        sourcePtr, audioController->getControllerId(), sourcePtr->mUpdateSettings);
...
void UpdateConstructor::applyCurrentControllerSettings(std::shared_ptr<SourceBase> source,
    std::shared_ptr<AudioController>                                               audioController,
    std::shared_ptr<std::map<std::string, std::any>>                               settings) {

  // There is no need to check for already set values here because this functions only gets called
  // when creating a new source, at which point there cannot be any previous settings.

  // run finalSetting through pipeline
  auto failedSettings =
      mProcessingStepsManager->process(source, audioController->getControllerId(), settings);
...
void UpdateConstructor::applyCurrentGroupSettings(std::shared_ptr<SourceBase> source,
    std::shared_ptr<AudioController>                                          audioController,
    std::shared_ptr<std::map<std::string, std::any>>                          settings) {
  ...
  // run finalSetting through pipeline
  auto failedSettings =
      mProcessingStepsManager->process(source, audioController->getControllerId(), finalSettings);
...
bool UpdateConstructor::containsRemove(std::shared_ptr<std::map<std::string, std::any>> settings) {
  for (auto it = settings->begin(); it != settings->end(); ++it) {
    if (it->second.type() == typeid(std::string) &&
        std::any_cast<std::string>(it->second) == "remove") {
      return true;
    }
  }
  return false;
...
void UpdateConstructor::rebuildPlaybackSettings(
    std::shared_ptr<AudioController> audioController, std::shared_ptr<SourceBase> source) {
  // take current controller settings
  auto finalSettings =
      std::make_shared<std::map<std::string, std::any>>(*(audioController->mCurrentSettings));
  ...
  if (!source->mGroup.expired()) {
    ...
    auto groupSettings = std::make_shared<std::map<std::string, std::any>>(
        *(source->mGroup.lock()->mCurrentSettings));
  ...
  auto sourceSettings =
      std::make_shared<std::map<std::string, std::any>>(*(source->mCurrentSettings));
