...
/// settings only contain things that are changing. Once the settings are computed, the UpdateConstructor
/// will call the pipeline and write the currently set settings to the controller, groups, sources and
...
class CS_AUDIO_EXPORT UpdateConstructor {
 ...
 private:
  ...
  /// @param audioController audioController on which the source lives.
  /// @param source source to update
  void rebuildPlaybackSettings(std::shared_ptr<AudioController> audioController, 
    std::shared_ptr<SourceBase> source);
