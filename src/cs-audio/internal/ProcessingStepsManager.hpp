...
class CS_AUDIO_EXPORT ProcessingStepsManager {
 public:
  ...
  std::shared_ptr<std::vector<std::string>> process(std::shared_ptr<SourceBase> source,
      int audioControllerId, std::shared_ptr<std::map<std::string, std::any>> sourceSettings);
 ...
 private:
  ...
  /// is part of mUpdateProcessingSteps. If so, removes the given processing step from
  /// mUpdateProcessingSteps.
