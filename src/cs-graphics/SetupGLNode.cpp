...
void GLAPIENTRY MessageCallback(GLenum source, GLenum type, GLuint id, GLenum severity,
  ...
  //Print the following infos (OpenGL errors, shader compile errors, perf. warnings, shader compilation warnings, depricated code, redundant state changes, undefined behaviour, anything that isnt an error or perf. issue)
  ...
  //Print the following infos (OpenGL errors, shader compile errors, perf. warnings, shader compilation warnings, depricated code, redundant state changes, undefined behaviour)
  if(settings->pLogLevelGL.get() <= spdlog::level::info
    && severity == GL_DEBUG_SEVERITY_LOW)
  {
   logger().info("{}", message);
  }

  //Print the following infos (OpenGL errors, shader compile errors, perf. warnings, shader compilation warnings, depricated code)
  if(settings->pLogLevelGL.get() <= spdlog::level::warn
...
{
  mSettings->pLogLevelGL.connectAndTouch(
      [](auto level) { 
