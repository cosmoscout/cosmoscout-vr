  ...
  class GuideApi extends IApi {
    ...
    #greet() {
      if (!this.#hasGreeted) {
        ...
        this.agent.speak("Hi, I am your guide, Clippy. I am here to help you navigate this large Universe.");
    ...
    #checkCenter() {
      if (CosmoScout.flyToLocations) {
        ...
        if (!this.#centerChangeDisplayed && prevCenter !== currCenter) {
          ...
          this.agent.moveTo(bookmarksRect.right + 50, (bookmarksRect.top + bookmarksRect.bottom - this.#clippyHeight) / 2);
          this.agent.gestureAt(bookmarksRect.left, (bookmarksRect.top + bookmarksRect.bottom - this.#clippyHeight) / 2);
          this.agent.speak("Hi, I see you moved away from a planet. You can use the bookmarks tab to move to places. This way you won't get lost in the infinite space of the Universe!");
          this.agent.gestureAt(bookmarksRect.left, (bookmarksRect.top + bookmarksRect.bottom - this.#clippyHeight) / 2);
    ...
    #planetRotateChecker() {
      ...
      if (!this.#planetRotated && !isSame(prevPos, currPos) && length2(prevPos) !== length2(currPos)) {
        this.#planetRotated = true;

        this.agent.play("GetAttention");
        this.agent.speak("I see you rotated the planet! You can interact in different ways with the planet.");
        {
          ...
          this.agent.speak("You can click the button with the compass to orient the camera so north is the up direction.");
        }
        {
          ...
          this.agent.moveTo((fixHorizonButtonRect.left + fixHorizonButtonRect.right - this.#clippyWidth) / 2, fixHorizonButtonRect.bottom);
        ...
        }
        {
          ...
          this.agent.moveTo((landOnSurfaceButtonRect.left + landOnSurfaceButtonRect.right - this.#clippyWidth) / 2, landOnSurfaceButtonRect.bottom);
          this.agent.play("GestureUp");
        ...
        }
        {
          ...
          this.agent.moveTo((launchToOrbitButtonRect.left + launchToOrbitButtonRect.right - this.#clippyWidth) / 2, launchToOrbitButtonRect.bottom);
          this.agent.play("GestureUp");
