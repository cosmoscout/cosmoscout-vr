...
class WCSPointSampleComponent extends Rete.Component {

  constructor() {
    // This name must match the WCSSourceNode::sName defined in WCSSource.cpp.
    super("WCSPointSample");
  ...
  builder(node) {
    ...
    node.addInput(coords);

    let timeInput = new Rete.Input('wcsTimeIn', "Time", CosmoScout.socketTypes['WCSTime']);
    node.addInput(timeInput);

    let imageOutput = new Rete.Output('imageOut', 'Image 1D', CosmoScout.socketTypes['Image1D']);
