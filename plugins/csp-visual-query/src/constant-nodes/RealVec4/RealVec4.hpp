...
#ifndef CSP_VISUAL_QUERY_RealVec4_NUMBER_NODE_HPP
#define CSP_VISUAL_QUERY_RealVec4_NUMBER_NODE_HPP

#include "../../../../csl-node-editor/src/Node.hpp"

namespace csp::visualquery {

/// The RealVec4 provides a user defined vector.
/// The node will write a new value to its output whenever the user edits the number and thus
/// trigger a reprocessing of the connected nodes. The current value is stored as a private member
/// and will be serialized and deserialized whenever the node graph is saved or loaded.
class RealVec4 : public csl::nodeeditor::Node {
 public:
  // static interface ------------------------------------------------------------------------------

  static const std::string         sName;
  static std::string               sSource();
  static std::unique_ptr<RealVec4> sCreate();
  ...
  /// Whenever the user changes the number value of the node, the RealVec4 will send a message to
 ...
 private:
  std::array<double, 4> mValue;
};

} // namespace csp::visualquery

#endif // CSP_VISUAL_QUERY_RealVec4_NUMBER_NODE_HPP