...
Axis::Axis(std::shared_ptr<Plugin::Settings> pluginSettings,
    ...
    std::shared_ptr<cs::graphics::ObjLoader> axisModel, const glm::dvec3 rotAxis,
  ...
  mVBO->BufferData(axisModel->getVertices().get()->size() * sizeof(float),
      axisModel->getVertices().get()->data(), GL_DYNAMIC_DRAW);
