...
void Plugin::init() {
  ...
  mGuiManager->getGui()->registerCallback("orientationTools.enableAxes",
      ...
      std::function([this](bool value) { mPluginSettings->mEnableAxes = value; }));
...
void Plugin::addArrowsGroup(
  ...
  // Creates the arrows for the X, Y and Z-axis and sets them to the visualize the object stated in
  // settings. Only does so if no setting is set to disable that.
  if (!(settings.second.mDisableX.has_value() && settings.second.mDisableX.value())) {
    auto arrowX = std::make_shared<Arrow>(mPluginSettings, mSolarSystem, mArrowModel, mRotAxisX,
        mAngleX, mColorX, settings.second.mSize * 0.1f);
  ...
  if (!(settings.second.mDisableY.has_value() && settings.second.mDisableY.value())) {
    auto arrowY = std::make_shared<Arrow>(mPluginSettings, mSolarSystem, mArrowModel, mRotAxisY,
        mAngleY, mColorY, settings.second.mSize * 0.1f);
  ...
  if (!(settings.second.mDisableZ.has_value() && settings.second.mDisableZ.value())) {
    auto arrowZ = std::make_shared<Arrow>(mPluginSettings, mSolarSystem, mArrowModel, mRotAxisZ,
        mAngleZ, mColorZ, settings.second.mSize * 0.1f);
...
void Plugin::addAxesGroup(
  ...
  // Creates two axes for each axis visualization. Rotates one axis to fit on the axis it shall
  // represent and the other 180° in the other direction.
  if (!(settings.second.mDisableX.has_value() && settings.second.mDisableX.value())) {
    auto axis = std::make_shared<Axis>(mPluginSettings, mSolarSystem, mAxisModel, mRotAxisX,
        mAngleX, glm::vec4(settings.second.mColor, 1.F), settings.second.mSize * 0.1f);
    axis->setParentName(settings.first);
    axisGroup.push_back(axis);
  }

  if (!(settings.second.mDisableY.has_value() && settings.second.mDisableY.value())) {
    auto axis = std::make_shared<Axis>(mPluginSettings, mSolarSystem, mAxisModel, mRotAxisY,
        mAngleY, glm::vec4(settings.second.mColor, 1.F), settings.second.mSize * 0.1f);
    axis->setParentName(settings.first);
    axisGroup.push_back(axis);
  }

  if (!(settings.second.mDisableZ.has_value() && settings.second.mDisableZ.value())) {
    auto axis = std::make_shared<Axis>(mPluginSettings, mSolarSystem, mAxisModel, mRotAxisZ,
        mAngleZ, glm::vec4(settings.second.mColor, 1.F), settings.second.mSize * 0.1f);
    axis->setParentName(settings.first);
    axisGroup.push_back(axis);
  }
