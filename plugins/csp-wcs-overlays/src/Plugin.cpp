...
void Plugin::onLoad() {
  ...
  for (auto& settings : mPluginSettings->mBodies) {
    ...
    auto anchor = mAllSettings->mObjects.find(settings.first);

    if (anchor == mAllSettings->mObjects.end()) {
    ...
    for (auto const& wcsUrl : settings.second.mWcs) {
      mWcsCreationThreads.at(settings.first).enqueue([this, settings, wcsUrl]() {
        try {
          csl::ogc::WebCoverageService wcs(wcsUrl, mPluginSettings->mUseCapabilityCache.get(),
  ...
  mSolarSystem->pActiveObject.touch(mActiveBodyConnection);
}
...
void Plugin::setWCSServer(
  ...
  auto const& server =
      std::find_if(mWcs.at(wcsOverlay->getCenter()).begin(), mWcs.at(wcsOverlay->getCenter()).end(),
          [&name](csl::ogc::WebCoverageService const& wcs) { return wcs.getTitle() == name; });
...
void Plugin::setWCSCoverage(
  ...
  auto&                                settings = getBodySettings(wcsOverlay);
  std::optional<csl::ogc::WebCoverage> coverage =
  ...
  if (isActiveOverlay(wcsOverlay)) {
    ...
    if (!coverage->getSettings().mTimeIntervals.empty()) {
      ...
      auto start =
          csl::ogc::utils::timeToString(intervals.begin()->mFormat, intervals.begin()->mStartTime);
      auto end =
          csl::ogc::utils::timeToString(intervals.begin()->mFormat, intervals.begin()->mEndTime);
...
void Plugin::goToBounds(csl::ogc::Bounds const& bounds) {
  ...
  double radius = mSolarSystem->getObjectByCenterName(mActiveOverlay->getCenter())->getRadii()[0];
  double heighty =
...
void Plugin::registerSidebarCallbacks() {
  ...
  mGuiManager->getGui()->registerCallback("wcsOverlays.goToDefaultBounds",
      ...
      std::function([this]() {
        ...
        csl::ogc::WebCoverage::Settings layerSettings =
  ...
  mGuiManager->getGui()->registerCallback(
      "wcsOverlays.goToFirstTime", "Go to the first available timestep.", std::function([this]() {
        ...
        auto start =
            csl::ogc::utils::timeToString(intervals.front().mFormat, intervals.front().mStartTime);
        auto end =
            csl::ogc::utils::timeToString(intervals.back().mFormat, intervals.back().mEndTime);
