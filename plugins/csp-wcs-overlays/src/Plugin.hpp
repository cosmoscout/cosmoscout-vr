...
class Plugin : public cs::core::PluginBase {
 public:
  ...
  struct Settings {
    ...
    cs::utils::DefaultProperty<WebCoverageService::CacheMode> mUseCapabilityCache{
        WebCoverageService::CacheMode::eNever};
 ...
 private:
  ...
  std::map<std::string, std::vector<WebCoverageService>>         mWcs;

  std::shared_ptr<TextureOverlayRenderer> mActiveOverlay;
  /// The currently active WebCoverageService for each center name.
  std::map<std::string, std::optional<WebCoverageService>> mActiveServers;
  /// The currently active WebMapLayer for each center name.
  std::map<std::string, std::optional<WebCoverage>> mActiveCoverages;
