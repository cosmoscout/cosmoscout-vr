    ...
    for (const CesiumGltf::Mesh& mesh : pModel->meshes) {
        for (const CesiumGltf::MeshPrimitive& primitive : mesh.primitives) {
            ...
            if (uvIt != primitive.attributes.end()) {
                uvs = CesiumGltf::AccessorView<CesiumGltf::AccessorTypes::VEC2<float>>(
                    *pModel, uvIt->second);
            ...
            if (primitive.material >= 0) {
                const auto* pMat = CesiumGltf::Model::getSafe(
                    &pModel->materials, primitive.material);
...
void* StubPrepareRendererResources::prepareInMainThread(
    ...
    logger().info("[Cesium] GPU Upload: {} vertices, {} indices.",
        pData->vertices.size() / 12, pData->indexCount);
    ...
    if (pData->hasTexture && !pData->texturePixels.empty()) {
        ...
        if (pData->texChannels == 1)      format = GL_RED;
        else if (pData->texChannels == 2) format = GL_RG;
        else if (pData->texChannels == 3) format = GL_RGB;
        ...
        logger().info("[Cesium] GPU Upload: Texture uploaded, ID={}, {}x{}.",
            pData->textureId, pData->texWidth, pData->texHeight);
...
void StubPrepareRendererResources::free(
    ...
    if (pMainThreadResult) {
        ...
        logger().debug("[Cesium] GPU Free: Deleted VAO={}, VBO={}, EBO={}, Tex={}.",
            pData->vao, pData->vbo, pData->ebo, pData->textureId);
