...
* **New Plugin Library: `csl-node-editor`.** With this library, plugins can provide an interface to create complex data flow graph. The node editor interface is made available as via a web server. Hence the node graph can be modified either within CosmoScout VR or on an external device.
